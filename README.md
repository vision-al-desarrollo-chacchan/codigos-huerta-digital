...
3. Configurar en Supabase los secretos de Google OAuth y las claves de cifrado.
4. Desplegar `gmail-oauth-start`, `gmail-oauth-callback` y `lookup-code`.
5. Copiar `.env.example` a `.env.local` y colocar la URL y la clave publicable.
6. Ejecutar `npm install` y `npm run build`.
7. Conectar el repositorio a Cloudflare Pages: comando `npm run build`, salida `dist`.
8. Añadir el dominio `codigos.huertadigital.net.pe`.
