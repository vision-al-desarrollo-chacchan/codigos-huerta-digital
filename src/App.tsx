...
function Lookup({onAdmin}:{onAdmin:()=>void}) {
  const [email,setEmail]=useState(''); const [platform,setPlatform]=useState(''); const [loading,setLoading]=useState(false)
...
function Admin({onBack,loggedIn}:{onBack:()=>void;loggedIn:boolean}) {
  ...
  const add=async(e:FormEvent)=>{e.preventDefault();setSaved('');const {error}=await supabase.from('code_assignments').insert({customer_email:customer.trim().toLowerCase(),platform_id:platform,code:code.trim()});if(error){setSaved('No se pudo guardar: '+error.message);return}setCustomer('');setCode('');setSaved('Código asignado correctamente.');load()}
