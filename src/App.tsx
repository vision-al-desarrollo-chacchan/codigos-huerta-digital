...
import { ChevronLeft, Copy, KeyRound, LogOut, Mail, Plus, Search, ShieldCheck, Tv, Users } from 'lucide-react'
...
function Admin({onBack,loggedIn}:{onBack:()=>void;loggedIn:boolean}) {
  ...
  const load=async()=>{const [{data:codes},{data:clientRows},{data:accountRows}]=await Promise.all([supabase.from('code_assignments').select('id,customer_email,code,status,created_at,platforms(name)').order('created_at',{ascending:false}).limit(30),supabase.from('clients').select('id,name,central_gmail,status,created_at').order('created_at',{ascending:false}),supabase.from('client_accounts').select('id,client_id,platform_id,account_email,active,platforms(name)').order('created_at',{ascending:false})]);setItems((codes as unknown as Assignment[])||[]);setClients((clientRows as Client[])||[]);setAccounts((accountRows as unknown as ClientAccount[])||[])}
