...
type ScriptResult={ok?:boolean;code?:string;messageId?:string;message?:string}
...
async function requestCode(email:string,platform:string){
  ...
  if(!response.ok||!result.ok||!result.code||!result.messageId)return {found:null,message:result.message||'No encontramos un código reciente.'}
  if(!/^(?:\d{4}|\d{6})$/.test(result.code))throw new Error('Código inválido recibido')
  return {found:{code:result.code,messageId:result.messageId},message:''}
}
